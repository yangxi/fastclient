...
class Results:
  ...
  def __init__(self, savFile):
    ...
    self.current.write("#taskID->int,totalHitCount->int,receiveStamp->long,waitingTime->long,processingTime->long,perfCounter0->long,perfCounter1->long,clientTime,clientStartTime,clientEndTime\n");
    self.nr_results = 0;
  def add(self, reply):
    self.nr_results += 1;
...
#    time.sleep(2);
  ...
  def send(self, startTime, task):
    taskString = str(self.taskID) + ";" + task;    
...
#    actualQPSStats = RollingStats(5)
    ...
    while True:
      ...
      while len(result) < RECV_BUFFER_SIZE:
        result = result + self.sock.recv(RECV_BUFFER_SIZE - len(result))
#      print result
      taskID, totalHitCount, receiveStamp, waitingTime, processTime,retiredIns, unhaltedCycles = result.split(':')
...
#        queriesThisSec += 1
      ...
      latencyUS = (endTime-taskStartTime)*(1000)
#      result.append(str(latencyMS));
#      queueTimeStats.add(queueTimeMS)
#      totalTimeStats.add(latencyMS)
      self.results.add(result+(":%.3f: %d: %d" % (latencyUS,taskStartTime * 1000 * 1000, endTime * 1000 * 1000)));
