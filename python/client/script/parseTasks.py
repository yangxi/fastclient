...
# serverQtime, serverPtime, and serverLatency are in MS
    cols[key_col["serverLatency"]] = (np.array(cols[key_col["finishTime"]])/(1000 * 1000.0));
    cols[key_col["serverQtime"]] = (np.array(cols[key_col["queueTime"]])/(1000 * 1000.0));
    cols[key_col["serverPtime"]] = cols[key_col["serverLatency"]] - cols[key_col["serverQtime"]];
# The CPU is running at 2GHz
    cpuPtime = np.array(cols[key_col["retiredCycles"]])/(2*1000*1000.0)
    ...
    client_time = cols[key_col["serverLatency"]];
