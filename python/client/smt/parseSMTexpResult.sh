...
#for bench in self chart; do
	...
	python ../script/parseTasks.py ./${bench}smtInvok${i}_*_20;
	cp qps-latency.csv ${bench}smt_invok${i}_latency.csv;
	cp client-time-max.csv ${bench}smt_invok${i}_taillatency.csv;
	cp top200.csv ${bench}smt_invok${i}_top200.csv;
    done
done
